...
const CreateOfferDialog = ({ open, onClose, onOfferCreated }) => {
    ...
    useEffect(() => {
        const fetchProviders = async () => {
            try {
                const res = await axios.get('/admin/users', { params: { role: 'provider' } });
    ...
    const handleSubmit = async () => {
        ...
        Object.entries(formData).forEach(([key, value]) => {
            if (key === 'images' && Array.isArray(value)) {
                value.forEach((file, index) => formDataToSend.append(`images[${index}]`, file));
            } else {
                formDataToSend.append(key, value);
            }
        });
        try {
            await axios.post('/admin/offers', formDataToSend, {
