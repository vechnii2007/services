...
const RequestsTab = () => {
    ...
    const handleChangeRequestStatus = async (requestId, newStatus) => {
        try {
            ...
            setSnackbar({ open: true, message: 'Request status updated', severity: 'success' });
        } catch (error) {
            setSnackbar({ open: true, message: 'Error changing request status', severity: 'error' });
        }
    };
        ...
        <TableRow key={request._id}>
            ...
            <TableCell>
                <Button
                    variant="contained"
                    color="primary"
                    onClick={() => { setSelectedRequestId(request._id); setOpenEditDialog(true); }}
                    sx={{ marginRight: 1 }}
                >
                    {t('edit')}
                </Button>
        ...
        <Box>
            ...
            <Box sx={{ display: 'flex', gap: 2, marginBottom: 2 }}>
                <FilterControls
                    ...
                    onSelectChange={(e) => { setRequestStatusFilter(e.target.value); setPage(1); }}
            ...
            {loading ? (
                ...
                <>
                    <GenericTable headers={headers} rows={requests} renderRow={renderRow} />
