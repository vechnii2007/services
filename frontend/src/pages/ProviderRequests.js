...
const ProviderRequests = () => {
    ...
    useEffect(() => {
        const fetchRequests = async () => {
            try {
                ...
                const res = await axios.get('http://localhost:5001/api/services/requests', {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
    ...
    const filteredRequests = requests.filter((request) => {
        return (
            ...
            (!filters.description || request.description.toLowerCase().includes(filters.description.toLowerCase()))
        );
    });
    ...
    const handleOfferSubmit = async (requestId) => {
        try {
            ...
            const res = await axios.post(
                'http://localhost:5001/api/services/offer',
                {
                    requestId,
