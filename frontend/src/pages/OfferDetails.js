...
const OfferDetails = () => {
    ...
    useEffect(() => {
        const fetchOffer = async () => {
            try {
                const res = await axios.get(`http://localhost:5001/api/services/offers/${id}`);
                setOffer(res.data);
            ...
            <Card>
                <CardContent>
                    <Typography variant="body1">
                        <strong>{t('type')}:</strong> {offer.type === 'independent' ? t('independent_offer') : t('service_offer')}
                    </Typography>
                    <Typography variant="body1">
                    ...
                    <Typography variant="body1">
                        <strong>{t('created_at')}:</strong> {new Date(offer.createdAt).toLocaleString()}
                    </Typography>
                    <Button
                        variant="contained"
                        color="primary"
